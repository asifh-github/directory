prostate <- read.csv("Datasets/Prostate.csv", header = TRUE, sep = ",",
                       na.strings = " ")
...
open3d()
persp3d(
  x = seq(from = -2, to = 4, by = .05), y = seq(from = 0, to = 100, by = .5),
  z = surface, col = "orange", xlab = "lcavol", ylab = "pgg45",
  zlab = "Predicted lpsa"
)
points3d(prostate$lpsa ~ prostate$lcavol + prostate$pgg45, col = "blue")

# Compare estimated regression coefficients from separate regs vs. multiple
