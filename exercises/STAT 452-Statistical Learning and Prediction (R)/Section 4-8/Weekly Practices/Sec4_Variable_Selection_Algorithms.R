...
#  Need to separately give data for response and explanatory
#
# regsubsets() will return information on the best models of each size
...
allsub1 <- regsubsets(
  x = prostate[prostate$set == 1, 1:8],
  y = prostate[prostate$set == 1, 9], nbest = 1
)
allsub2 <- regsubsets(
  x = prostate[prostate$set == 2, 1:8],
...
par(mfrow = c(1, 3))
plot(
  x = results1[, 1], y = results1[, 2], xlab = "Vars in model", ylab = "sample-MSE",
  main = "SampleMSE vs Vars: 1st", type = "b"
)
plot(
  x = results1[, 1], y = results1[, 3], xlab = "Vars in model", ylab = "BIC",
  main = "BIC vs Vars: 1st", type = "b"
)
plot(
  x = results1[, 1], y = results1[, 4], xlab = "Vars in model", ylab = "MSPE",
...
step1 <- step(
  object = initial.1, scope = list(upper = final.1),
  k = log(n1)
)
...
step2 <- step(
  object = initial.2, scope = list(upper = final.2),
  k = log(n2)
)
