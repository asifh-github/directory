#########################################################################
# Ridge regression (for good predictions, does not get rid of predictors) 
# and LASSO (for choosing X and good prediction, may remove some of the predictors) 
# with Prostate Data
...
pred.ri2 <- as.matrix(cbind(1, prostate[set == 2, 1:8])) %*% coef.ri.best2
MSPE2.ridge <- mean((prostate[set == 2, 9] - pred.ri2)^2)
