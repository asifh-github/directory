...
covid <- read.csv("Datasets/Covid-BC-2020-09-22.csv", header = TRUE)
...
legend(
  x = 18300, y = 170, legend = c("N-Fold CV", "Generalized CV"),
...
legend(
  x = 18290, y = 170, legend = c(
    "Polynomial", "Cubic reg Spline", "Natural Spline",
...
lines(x = covid$date, y = predict(poly12, newdata = covid), 
      col = colors()[121], lwd = 2)

cub.spl.12 <- lm(data = covid, Cases ~ bs(date, df = 12))
lines(x = covid$date, y = predict(cub.spl.12, newdata = covid), 
      col = colors()[145], lwd = 2)

nat.spl.12 <- lm(data = covid, Cases ~ ns(date, df = 12))
lines(x = covid$date, y = predict(nat.spl.12, newdata = covid), 
      col = colors()[84], lwd = 2)
...
plot(
  x = covid$date[1:150], y = covid$Cases[1:150], type = "l", col = "gray",
  main = "Comparison of 12-DF fits"
)
legend(
  x = 18300, y = 80, legend = c(
    "Polynomial", "Cubic reg Spline", "Natural Spline",
    "Smoothing Spline", "LOESS"
  ),
  lty = "solid", col = colors()[c(121, 145, 84, 53, 24)], lwd = 2
...
poly12 <- lm(data = covid, Cases ~ poly(x = date, degree = 12))
lines(x = covid$date[1:150], y = predict(poly12, newdata = covid)[1:150],
      col = colors()[121], lwd = 2)
...
nat.spl.12 <- lm(data = covid, Cases ~ ns(date, df = 12))
lines(x = covid$date[1:150], y = predict(nat.spl.12, newdata = covid)[1:150], 
      col = colors()[84], lwd = 2)
